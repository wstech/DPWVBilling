...
public class TariffRateCalculatorReeferSunday extends AbstractTariffRateCalculatorInterceptor {
	...
	public void calculateRate(Map inOutMap) {
		 ...
		 while( !start.after(end)){
			...
			if (searchCalendarSunday(targetDay)){
				if(searchCalendarPublicHolidays(targetDay,calendar) ){
					...
					if (overtime==false) {
						qtyDays++;
						logUsr.append("SUN=").append(fmt.format(targetDay)).append("\n");
		...
		logEx("Calculating Monitoring Sunday: eventChargesAmountTotal:" + outAmount, inv);
